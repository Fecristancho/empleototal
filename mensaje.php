...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['enviar_mensaje'])) {
    ...
    header("Location: mensaje.php?id_receptor=$id_receptor"); // Redirigir para evitar reenvío del formulario
    exit();
}
...
$stmt = $base_de_datos->prepare("
    ...
    FROM usuario u
    JOIN mensajes m ON (m.id_receptor = u.id_usuario OR m.id_interceptor = u.id_usuario)
    WHERE (m.id_receptor = :id_usuario OR m.id_interceptor = :id_usuario) AND u.usuario LIKE :busqueda
");
$stmt->execute(['id_usuario' => $id_usuario, 'busqueda' => '%' . $busqueda . '%']);
...
<body>
    ...
    <header>
        <div class="container">
            ...
            <div class="user-icons">
                ...
                <button class="btn-lila" onclick="location.href='./notificaciones.php'">Notificaciones</button>
    ...
    <section class="container-fluid main-content d-flex flex-column flex-md-row pt-5">
                ...
                <ul class="list-group">
                    <?php foreach ($usuarios_conversacion as $usuario): ?>
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <?php endif; ?>
                            <a href="?id_receptor=<?= $usuario['id_usuario'] ?>" class="btn btn-primary btn-sm">Ver Chat</a>
                        </li>
                    <?php endforeach; ?>
                </ul>
            <?php endif; ?>
        ...
        <article class="col-12 col-md-8 message-box bg-white rounded">
            <h3 id="chatUser"><?= $id_receptor ? 'Chat con ' . htmlspecialchars(array_column($usuarios_conversacion, 'usuario', 'id_usuario')[$id_receptor]) : 'Selecciona un usuario para iniciar el chat' ?></h3>
            <div id="chatBox">
            ...
            <?php if ($id_receptor): ?>
                <form method="POST" action="mensaje.php" id="messageForm">
